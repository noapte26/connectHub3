...
public class NewsFeedWindow extends javax.swing.JFrame {
    ...
    private void createStoryPanel(Story story) {
        ...
        // Story Text Content
        String content = (story.getContent() != null && !story.getContent().isEmpty()) ? story.getContent() : "No content available";
        JLabel contentLabel = new JLabel("[" + getname(story.getAuthorId()) + "]" + content);
        contentLabel.setBackground(Color.WHITE);
        contentLabel.setOpaque(true);

        // Set fixed size for the story panel
        //contentLabel.setPreferredSize(new Dimension(90, 70));
        storyPanel.add(contentLabel, BorderLayout.CENTER);

        // Image Preview (Optional)
        if (story.getImagePath() != null) {

            ImageIcon imageIcon = new ImageIcon(story.getImagePath());
            Image image = imageIcon.getImage().getScaledInstance(60, 60, Image.SCALE_SMOOTH); // Adjust image to fit within 90x70
            contentLabel.setIcon(new ImageIcon(image));
            contentLabel.setHorizontalAlignment(SwingConstants.CENTER);
            contentLabel.add(Box.createRigidArea(new Dimension(0, 10)));
        }
    ...
    private void createPostPanel(Post post) {
        ...
        JLabel contentLabel = new JLabel("[" + getname(post.getAuthorId()) + "]" + content);
        contentLabel.setBackground(Color.WHITE);
        contentLabel.setOpaque(true);
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        for (UserAccount acc : accounts) {
            if (acc.getUser().getUserId() == authorId) {
    ...
    private void suggestsMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_suggestsMouseClicked
        for (UserAccount acc : accounts) {
            if (acc.getUser().getUserId() == authorId) {
                FriendSuggestionsWindow a1 = new FriendSuggestionsWindow(acc);
                a1.setVisible(true);
            }
        }
    }//GEN-LAST:event_suggestsMouseClicked
    ...
    private void jLabel10MousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jLabel10MousePressed
            ...
            if (acc.getUser().getUserId().equals(authorId)) {
                ...
                ProfileWindow a1 = new ProfileWindow (acc.getProfile());
